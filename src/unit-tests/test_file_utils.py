...
def test_group_by(data_set):       
       assert futl.group_by(data_set, group_columns=['Name', 'Year'], show_columns=["Name", "Year"], aggregation="count") == [
       ...
       with raises(Exception) as e:        
              assert futl.group_by(data_set, group_columns=['Name', 'Year'], show_columns=["Name", "Year", "Score"], aggregation="count") 
